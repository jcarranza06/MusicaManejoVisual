...
hablar("vamos a entrenar el modelo, mire hacia arriba")
while True:
    ret, frame = cap.read()
    added = False
    if ret == False:
        ...
        #Dibujando los rectangulos de detecciona de los ojos
            ...
            for contorno in contornos:
                ...
                if(len(predicciones)> 7):
                    print(predicciones)
                    p = model.predict(np.array(predicciones))
                    print(p)
                    p = np.mean(p, axis=0)
                    print(p)
                    cv2.putText(roi_color, lados[np.argmax(p)]["name"], (x1, y1), cv2.FONT_HERSHEY_COMPLEX,1,(0,255,0),2)
                    hablar(lados[np.argmax(p)]["name"])
                    predicciones=[]
